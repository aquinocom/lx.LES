## Script (Python) "custom-livescript_reply"
...
quoted_term = url_quote_plus(q)

searchterm_query = '?searchterm=%s' % quoted_term
...
if not results:
    ...
    #se nada for encontrado, sugere a criação de um novo paciente.
    write('''<div class="LSRow">''')
    write('<img width="16" height="16" src="++resource++lx.LES.images/paciente.png" alt="Adicionar paciente" />')
    write('<a href="%s/createObject?type_name=Paciente">%s</a>' % (portal_url,"[Adicionar Novo paciente]"))
    write('''</div>''')

    write('''<div class="LSRow">''')
    write('<a href="%s&path=%s" class="advancedsearchlink">%s</a>' %
            (portal_url + '/@@custom-search?advanced_search=True&portal_type%3Alist=Paciente&SearchableText='+quoted_term,
            params['path'],
    ...
    write('''<li class="LSRow">''')
    write('<a href="%s&path=%s" class="advancedsearchlink advanced-search">%s</a>' %
            (portal_url + '/@@custom-search?advanced_search=True&portal_type%3Alist=Paciente&SearchableText='+quoted_term,
            params['path'],
            ts.translate(label_advanced_search, context=REQUEST)))
    write('''</li>''')
    ...
    if len(results) > limit:
        # add a more... row
        write('''<li class="LSRow">''')
        searchquery = '@@custom-search?portal_type%3Alist=Paciente&SearchableText=%s&path=%s' \
